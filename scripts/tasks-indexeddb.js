	...
	return {
		...
		saveAll : function(type, objArray, successCallback, errorCallback){
			...
			$.each(objArray, function(i, obj){
				...
				request.onsuccess = function (event){
					//obj.id = event.target.result; //not used
				};
				request.onerror = function(event){
					errorCallback('object_not_stored', 'It is not possible to store the objects');
				};
			});

		},
        deleteAll: function(type,successCallback,errorCallback){ //try to delete all rows || NEW FROm W8
            var tx = database.transaction([type], "readwrite");
            tx.oncomplete = function(event){successCallback();};
            tx.onerror = function(event) {
	    		console.log(event);
	    		errorCallback('transaction_error', 'It is not possible to store the object');
	    	};
